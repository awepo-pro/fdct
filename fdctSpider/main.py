...
def main():
    if arg.download:
        ...
        output_paths = ['./info/fund.json', './info/bulletin.json', './info/bulletin_detail.json', './info/contact_us.json', './info/transformation_case.json',
                        './info/mainland_coop.json', './info/other_coop.json', './info/expert_database.json']
        update_file(output_paths)
        check_file(['log/'])
        
        println('start fetching information about funding')
        os.system('scrapy crawl fund -o ' + output_paths[0] + ' -t json 2>> log/info.txt')
        
        println('start fetching information about bulletin')
        os.system('scrapy crawl bulletin -o ' + output_paths[1] + ' -t json 2>> log/info.txt')
        
        println('start fetching information about bulletin detail')
        os.system('scrapy crawl bulletin_detail -o ' + output_paths[2] + ' -t json 2>> log/info.txt')

        println('start fetching information about contact us')
        os.system('scrapy crawl contact_us -o ' + output_paths[3] + ' -t json 2>> log/info.txt')
        
        println('start fetching information about transformation case')
        os.system('scrapy crawl transformation_case -o ' + output_paths[4] + ' -t json 2>> log/info.txt')
        
        println('start fetching information about mainland-macau cooperation')
        os.system('scrapy crawl mainland_coop -o ' + output_paths[5] + ' -t json 2>> log/coop.txt')
        
        println('start fetching information about other country and macau cooperation')
        os.system('scrapy crawl other_coop -o ' + output_paths[6] + ' -t json 2>> log/coop.txt')
        
        println('start fetching information about experct database')
        os.system('scrapy crawl expert_database -o ' + output_paths[7] + ' -t json 2>> log/coop.txt')
